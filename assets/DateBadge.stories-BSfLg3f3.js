import{j as V}from"./jsx-runtime-u17CrQMm.js";import{D as _}from"./DateBadge-Calkfi_n.js";import{A as Y}from"./AppContext-CgiYEWF5.js";import{t as U,D as P}from"./tasks-BV5_RF93.js";import"./edge-cases-Dobf_xol.js";import{w as l,d as c}from"./test-utils-BNouzmjp.js";import"./iframe-KW6RtBM2.js";import"./preload-helper-PPVm8Dsz.js";import"./Icon-CFZCLKt0.js";import"./Motion-DxXHqm4t.js";import"./proxy-B9VYbwnC.js";import"./popover-DUPQEFUd.js";import"./index-CuxhDgIS.js";import"./index-BH6JDbuJ.js";import"./index-goBt61iM.js";import"./calendar-BTs5a-KE.js";import"./index-CuGcjXI2.js";const{expect:i,userEvent:o}=__STORYBOOK_MODULE_TEST__,ce={title:"UI/DateBadge",component:_,tags:["autodocs"],parameters:{layout:"centered"},argTypes:{onUpdate:{action:"task-updated"}},decorators:[n=>V.jsx(Y,{children:V.jsx("div",{className:"p-4 bg-slate-50 min-w-50",children:V.jsx(n,{})})})]},s=P.now(),F=s.plus({days:1}),m=s.minus({days:1}),W=U.base({id:"1",title:"Sample Task",dueAt:s.toISODate()}),H=n=>console.log("Updated task:",n),r={args:{task:W,onUpdate:H,type:"dueDate",date:s.toISODate(),label:"Today",icon:"Calendar",className:"flex items-center gap-1 px-1.5 py-0.5 text-[10px] rounded border text-rose-600 bg-rose-50/80 border-rose-100/50"}},y={args:{...r.args,date:F.toISODate(),label:"Tomorrow",className:"flex items-center gap-1 px-1.5 py-0.5 text-[10px] rounded border text-amber-600 bg-amber-50/80 border-amber-100/50"}},g={args:{...r.args,date:m.toISODate(),label:m.toFormat("MMM d"),className:"flex items-center gap-1 px-1.5 py-0.5 text-[10px] rounded border text-rose-700 bg-rose-100 border-rose-200 font-bold"}},b={args:{...r.args,date:void 0,label:"No date",className:"flex items-center gap-1 px-1.5 py-0.5 text-[10px] rounded border text-slate-400 bg-slate-50 border-slate-100"}},d={args:{task:U.base({startAt:s.toISO()}),onUpdate:H,type:"startAt",date:s.toISO(),label:"Today 9:00 AM",icon:"PlayCircle",className:"flex items-center gap-1 px-1.5 py-0.5 text-[10px] rounded border text-blue-600 bg-blue-50/80 border-blue-100/50"}},D={args:{...d.args,date:s.set({hour:14,minute:30}).toISO(),label:"Today 2:30 PM"}},v={args:{...d.args,date:void 0,label:"No start",className:"flex items-center gap-1 px-1.5 py-0.5 text-[10px] rounded border text-slate-400 bg-slate-50 border-slate-100"}},p={args:{task:U.base({createdAt:m.toISO()}),onUpdate:H,type:"createdAt",date:m.toISO(),label:"Yesterday 3:45 PM",icon:"Plus",className:"flex items-center gap-1 px-1.5 py-0.5 text-[10px] rounded border text-slate-500 bg-slate-50/80 border-slate-100/50"}},w={args:{...p.args,date:s.toISO(),label:"Today 9:00 AM"}},u={args:{task:U.completed({completedAt:s.toISO()}),onUpdate:H,type:"completedAt",date:s.toISO(),label:"Today 4:20 PM",icon:"CheckCircle2",className:"flex items-center gap-1 px-1.5 py-0.5 text-[10px] rounded border text-emerald-600 bg-emerald-50/80 border-emerald-100/50"}},S={args:{...u.args,date:m.toISO(),label:"Yesterday 11:15 AM"}},x={args:{...r.args,prefix:"DUE: "}},h={args:{...d.args,prefix:"START: "}},O={args:{...r.args},play:async({canvasElement:n,step:a})=>{const t=l(n);await a("Open date picker popover",async()=>{const e=t.getByRole("button");await o.click(e),await c(200)}),await a("Verify date input is visible",async()=>{const e=t.getByDisplayValue(s.toISODate());i(e).toBeInTheDocument(),i(e).toHaveAttribute("type","date")})}},I={args:{...d.args},play:async({canvasElement:n,step:a})=>{const t=l(n);await a("Open datetime picker popover",async()=>{const e=t.getByRole("button");await o.click(e),await c(200)}),await a("Verify datetime input is visible",async()=>{const e=t.getByDisplayValue(s.toFormat("yyyy-MM-dd'T'HH:mm"));i(e).toBeInTheDocument(),i(e).toHaveAttribute("type","datetime-local")})}},B={args:{...r.args},play:async({canvasElement:n,step:a})=>{const t=l(n);await a("Open date picker",async()=>{const e=t.getByRole("button",{name:/Change Due Date/i});await o.click(e),await c(200)}),await a("Change date value",async()=>{const e=t.getByDisplayValue(s.toISODate());await o.clear(e),await o.type(e,F.toISODate()),await c(100)}),await a("Save new date",async()=>{const e=t.getByRole("button",{name:/Save/i});await o.click(e),await c(100)})}},T={args:{...d.args},play:async({canvasElement:n,step:a})=>{const t=l(n);await a("Open datetime picker",async()=>{const e=t.getByRole("button",{name:/Change Start Date/i});await o.click(e),await c(200)}),await a("Change datetime value",async()=>{const e=t.getByDisplayValue(s.toFormat("yyyy-MM-dd'T'HH:mm")),L=F.set({hour:10,minute:0});await o.clear(e),await o.type(e,L.toFormat("yyyy-MM-dd'T'HH:mm")),await c(100)}),await a("Save new datetime",async()=>{const e=t.getByRole("button",{name:/Save/i});await o.click(e),await c(100)})}},k={args:{...r.args},play:async({canvasElement:n,step:a})=>{const t=l(n);await a("Open date picker",async()=>{const e=t.getByRole("button");await o.click(e),await c(200)}),await a("Clear date value",async()=>{const e=t.getByDisplayValue(s.toISODate());await o.clear(e),await c(100)}),await a("Save empty date (removes date)",async()=>{const e=t.getByRole("button",{name:/Save/i});await o.click(e),await c(100)})}},f={args:{...r.args,label:"A very long date label that might overflow the badge container"}},E={args:{...r.args,label:""}},C={args:{...r.args,date:"invalid-date-string",label:"Invalid"}},A={args:{...r.args,date:P.local(2099,12,31).toISODate(),label:"Dec 31, 2099"}},M={args:{...p.args,date:P.local(2e3,1,1).toISO(),label:"Jan 1, 2000"}},N={args:{...r.args},play:async({canvasElement:n,step:a})=>{const t=l(n);await a("Tab to badge button",async()=>{const e=t.getByRole("button");e.focus(),i(e).toHaveFocus()}),await a("Press Enter to open popover",async()=>{await o.keyboard("{Enter}"),await c(200);const e=t.getByDisplayValue(s.toISODate());i(e).toBeInTheDocument()})}},R={args:{...r.args},play:async({canvasElement:n,step:a})=>{const t=l(n);await a("Verify button has accessible title",async()=>{const e=t.getByRole("button",{name:/Change Due Date/i});i(e).toHaveAttribute("title","Change Due Date")})}};r.parameters={...r.parameters,docs:{...r.parameters?.docs,source:{originalSource:`{
  args: {
    task: defaultTask,
...
}`,...R.parameters?.docs?.source}}};const de=["DueDate","DueTomorrow","Overdue","NoDueDate","StartDate","StartDateWithTime","NoStartDate","CreatedDate","CreatedToday","CompletedDate","CompletedYesterday","WithPrefix","WithPrefixStartDate","DateOnlyInput","DateTimeInput","ChangeDueDate","ChangeStartDate","ClearDate","VeryLongLabel","EmptyLabel","InvalidDate","FarFutureDate","FarPastDate","KeyboardNavigation","AriaLabels"];export{R as AriaLabels,B as ChangeDueDate,T as ChangeStartDate,k as ClearDate,u as CompletedDate,S as CompletedYesterday,p as CreatedDate,w as CreatedToday,O as DateOnlyInput,I as DateTimeInput,r as DueDate,y as DueTomorrow,E as EmptyLabel,A as FarFutureDate,M as FarPastDate,C as InvalidDate,N as KeyboardNavigation,b as NoDueDate,v as NoStartDate,g as Overdue,d as StartDate,D as StartDateWithTime,f as VeryLongLabel,x as WithPrefix,h as WithPrefixStartDate,de as __namedExportsOrder,ce as default};
